...
impl Sweep{
    pub fn new(T_start:f64,T_end:f64,num_steps:u64)->Self{
        ...
        return Sweep { T_start, T_end, num_steps, delta_T};
    }
    pub fn update_T(&mut self){self.delta_T = (self.T_end-self.T_start)/(self.num_steps as f64);}
    ...
    pub fn T_start(&self)->f64{return self.T_start}
    pub fn T_end(&self)->f64{return self.T_end}
    pub fn num_steps(&self)->u64{return self.num_steps}
...
impl GridPos {
    ...
    pub fn pos(&self)->&Vec<u16>{return &self.pos}
    pub fn particle(&self)->Option<&Particle>{return self.particle.as_ref()}
    pub fn particle_as_mut(&mut self)->Option<&mut Particle>{return self.particle.as_mut()}
}
...
impl Grid{
    ...
    pub fn link_particles(&mut self, grid_id1:u32, grid_id2: u32)
        ...
        let link_from1_to2 = self.grid_positions[grid_id1 as usize].particle()
            .unwrap().find_link(grid_id2);
        let link_from2_to1 = self.grid_positions[grid_id2 as usize].particle()
            .unwrap().find_link(grid_id1);
        ...
        if link_from1_to2.is_some() && link_from2_to1.is_none(){
            ...
            self.grid_positions[grid_id2 as usize].particle_as_mut().unwrap().link_neighbour(
                Link::new_from(grid_id1, &link_from1_to2.unwrap()));
            return Ok(self);
        }
        ...
        if link_from2_to1.is_some() && link_from1_to2.is_none(){
            ...
            self.grid_positions[grid_id1 as usize].particle_as_mut().unwrap().link_neighbour(
                Link::new_from(grid_id2, &link_from2_to1.unwrap()));
            return Ok(self);
        }
        ...
        let coupling = match self.config.coupling_limits[0] == self.config.coupling_limits[1]{
            ...
            false => rand::thread_rng().gen_range(
                self.config.coupling_limits[0]..
                self.config.coupling_limits[1]),
        };

        //create link
        let link_from1_to2 = Link::new(
            grid_id2,
            coupling);
        //copy link from first link
        let link_from2_to1 = Link::new_from(grid_id1, &link_from1_to2);
        //set links
        self.grid_positions[grid_id1 as usize].particle_as_mut().unwrap().link_neighbour(link_from1_to2);
        self.grid_positions[grid_id2 as usize].particle_as_mut().unwrap().link_neighbour(link_from2_to1);
        return Ok(self)
    ...
    pub fn calc_energy(&self)->f64{
        ...
        for position in self.grid_positions.iter(){
            ...
            for neighbour_link in neighbour_links{
                ...
                energy -= 0.5*neighbour_link.coupling()*
                          f64::from(spin)*
                          f64::from(neighbour_spin);
            }
    ...
    pub fn metropolis_step(&mut self)->Option<usize>{
        ...
        let grid_id = rand::thread_rng().gen_range(
            0..usize::try_from(self.capacity).unwrap()
        );
        ...
        for neighbour_link in neighbour_links{
            ...
            dH += 2.0*neighbour_link.coupling()*
                f64::from(spin)*
                f64::from(neighbour_spin);
        };
        ...
        match rand::thread_rng().gen_bool(metro_p){
            false => return None,
            true => {
                self.grid_positions[grid_id].particle.as_mut().unwrap().flip_spin();
                return Some(grid_id)
            },
        }
    ...
    fn calc_filled_neighbour_indices(&self, grid_id: u32)->Vec<u32>{
        ...
        let neighbour_positions = 
            self.calc_neighbour_positions(grid_id);
    ...
    pub fn set_T(&mut self, T:f64){self.T = T}
    pub fn external_field(&self)->f64{return self.external_field}
    ...
    pub fn capacity(&self)->u32{return self.capacity}
    pub fn set_sweep(&mut self, sweep:Sweep){
        self.T = sweep.T_start;
        self.sweep = Some(sweep)}
    pub fn sweep(&self)->Option<&Sweep>{return self.sweep.as_ref()}
}

use std::{fmt, cmp::min, fs::File, io::Write, collections::VecDeque};
